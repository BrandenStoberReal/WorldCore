...
export function MobileModal({ open, onClose, title, children, className }: MobileModalProps) {
  ...
  return (
    <div
      className={cn(
        'modal-overlay bg-background/80 fixed inset-0 z-50 flex items-end justify-center backdrop-blur-sm',
        !open && 'modal-closed',
      )}
    ...
    >
      <div
        ref={containerRef}
        className={cn(
          'modal-content relative flex max-h-[90vh] w-full flex-col overflow-y-auto rounded-t-2xl',
          'border-border bg-card text-card-foreground border',
          'shadow-[0_-12px_40px_-8px_color-mix(in oklch,var(--ember)_30%,transparent)]',
      ...
      >
        ...
        <div className="border-border/60 bg-background/40 flex items-center justify-between border-t px-4 py-2.5">
