...
export function ChatView({ characterId }: ChatViewProps) {
  ...
  const findExistingChat = useCallback(() => {
    ...
    const res = fetch("/api/v1/chats/listByCharacter", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {activeChatId ? `${displayMessages.length} messages` : "No chat"}
            </p>
          </div>
        </div>
        <Button
        ...
        {displayMessages.length === 0 ? (
          <div className="flex h-full items-center justify-center">
            <div className="max-w-md text-center">
              ...
              <h4 className="mb-1 font-semibold">Start chatting with {character.name}</h4>
              {character.first_mes && (
                <div className="mt-3 rounded-lg bg-muted px-3 py-2 text-left text-sm">
                  <p className="whitespace-pre-wrap">{character.first_mes}</p>
                </div>
              )}
            </div>
          </div>
