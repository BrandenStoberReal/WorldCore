  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="h-px w-6 bg-ember/30" />
        <span className="text-[13px] font-semibold tracking-tight text-foreground/80">
          {label}
        </span>
      </div>
      {open ? (
        <ChevronUp className="h-3.5 w-3.5 text-muted-foreground/50 group-hover/section:text-ember transition-colors" />
      ) : (
        <ChevronDown className="h-3.5 w-3.5 text-muted-foreground/50 group-hover/section:text-ember transition-colors" />
      )}
    </button>
  );
}
...
export function ConnectionProfileForm({
  ...
  return (
    <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
      {/* ── Basic ── */}
      <Card className="rounded-sm py-0 overflow-hidden border-border/60">
        ...
        {openSections.basic && (
          <CardContent className="px-4 md:px-5 pb-4 pt-0 space-y-3 border-t border-border/40">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={form.api}
                onValueChange={(val) => updateField("api", val)}
              >
      ...
      <Card className="rounded-sm py-0 overflow-hidden border-border/60">
        ...
        {openSections.exclude && (
          <CardContent className="px-4 pb-4 pt-0 border-t border-border/40">
            <p className="text-[12px] text-muted-foreground/55 mt-3 mb-3">
              ...
              {EXCLUDE_OPTIONS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "mono-tag px-2 py-1 rounded-sm border text-[12px] transition-all",
                      active
