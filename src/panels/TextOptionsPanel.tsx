...
export function TextOptionsPanel() {
  ...
  const loadPreset = (category: string, presetName: string) => {
    ...
    if (category === "sysprompt") {
      ...
      setForm((f) => ({
        ...f,
        context: { ...f.context, selectedPreset: presetName, storyString: ctx.story_string, chatStart: ctx.chat_start, exampleSeparator: ctx.example_separator },
      }))
      ...
      setForm((f) => ({
        ...f,
        reasoning: { ...f.reasoning, selectedPreset: presetName, prefix: r.prefix, suffix: r.suffix, separator: r.separator },
      }))
    }
  }
          ...
          <InlineSection panelId="textoptions" sectionId="context" title="Context Template">
            <div className="space-y-4">
              ...
              <div className="space-y-2 pt-2 border-t border-border/50">
                <h5 className="text-[13px] font-semibold text-muted-foreground">Context Formatting</h5>

                <div className="space-y-1.5">
                  <Label className="text-[13px] font-medium">Story String Position</Label>
                  <Select value={form.context.storyStringPosition} onValueChange={(v) => setForm((f) => ({ ...f, context: { ...f.context, storyStringPosition: v as "default" | "inchat" } }))}>
                    <SelectTrigger className="w-full">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) => setForm((f) => ({ ...f, context: { ...f.context, storyStringDepth: parseInt(e.target.value) || 0 } }))}
                        className="font-mono text-[13px]"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-[13px] font-medium">Role</Label>
                      <Select value={form.context.storyStringRole} onValueChange={(v) => setForm((f) => ({ ...f, context: { ...f.context, storyStringRole: v as "system" | "user" | "assistant" } }))}>
                        <SelectTrigger className="w-full">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                ...
                {CONTEXT_CHECKBOXS.map(([key, label]) => (
                  <label key={key} className="flex items-center gap-2 text-[13px]">
                    <input
                      ...
                      onChange={(e) => setForm((f) => ({ ...f, context: { ...f.context, [key]: e.target.checked } }))}
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-2 text-[13px]">
                <input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, instruct: { ...f.instruct, bindToContext: e.target.checked } }))}
                  className="rounded border-border"
                />
                Bind to Context
              </label>
          ...
          <InlineSection panelId="textoptions" sectionId="sysprompt" title="System Prompt" defaultOpen>
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.tokenizer} onValueChange={(v) => setForm((f) => ({ ...f, tokenizer: v }))}>
                  <SelectTrigger className="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, tokenPadding: parseInt(e.target.value) || 0 }))}
                  className="font-mono text-[13px]"
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                {REASONING_CHECKBOXES.map(([key, label]) => (
                  <label key={key} className="flex items-center gap-2 text-[13px]">
                    <input
                      ...
                      onChange={(e) => setForm((f) => ({ ...f, reasoning: { ...f.reasoning, [key]: e.target.checked } }))}
                      className="rounded border-border"
                    />
                    {label}
                  </label>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, reasoning: { ...f.reasoning, maxAdditions: parseInt(e.target.value) || 0 } }))}
                  className="font-mono text-[13px] w-24"
                />
              </div>
            </div>
          </InlineSection>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, markdownEscapeStrings: e.target.value }))}
