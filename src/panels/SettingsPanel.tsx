...
const fields: { key: keyof SettingsForm; label: string; type?: string; caption: string }[] = [
  ...
  { key: 'top_k', label: 'Top K', type: 'number', caption: 'top-k sampling pool' },
  {
    ...
    type: 'number',
    caption: 'nudge repetition',
  },
  {
    key: 'presence_penalty',
    label: 'Presence Penalty',
    type: 'number',
    caption: 'encourage new tokens',
  },
  {
    key: 'frequency_penalty',
    label: 'Frequency Penalty',
    type: 'number',
    caption: 'decay frequent tokens',
  },
...
export function SettingsPanel() {
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-3">
        ...
        <span className="mono-tag text-muted-foreground/55">tempering parameters</span>
      </div>
    );
  }
  ...
  return (
    <div data-panel="settings" className="flex h-full flex-col gap-3">
      {/* Section header */}
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="display-host text-[42px] leading-none tracking-tight">Forge Parameters</h2>
          <p className="text-muted-foreground mt-2 max-w-md text-sm">
            Generation dials that shape responses. Sliders, penalties, and stop points for the LLM
            backend.
          </p>
        </div>
      ...
      {/* Parameters grid */}
      <InlineSection panelId="settings" sectionId="generation" title="Generation" defaultOpen>
        <div className="grid gap-x-4 gap-y-4 sm:grid-cols-2">
          {fields.map((f, idx) => (
            <div key={f.key} className="space-y-1.5">
              ...
              />
              <span className="mono-tag text-muted-foreground/50">{f.caption}</span>
            </div>
          ))}
        </div>
      </InlineSection>
      ...
      <InlineSection panelId="settings" sectionId="raw" title="Raw Manifest">
        {settings ? (
          <pre
            className="dot-grid m-0 max-h-72 overflow-auto rounded-sm p-3 font-mono text-[11.5px] leading-snug"
            style={{
          ...
          <div className="text-muted-foreground/55 py-3 text-[12px] italic">
