...
async function findOrCreateUser(
  ...
  const existing = await db.select().from(users).where(eq(users.id, username)).limit(1);
...
export async function verifyBasicAuth(
  ...
  const match = configuredUsers.find((u) => u.username === username && u.password === password);
