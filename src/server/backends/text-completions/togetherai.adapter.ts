...
export class TogetherAIAdapter implements TextCompletionAdapter {
  source = "togetherai" as const

  async forwardRequest(req: TextCompletionRequest): Promise<Response> {
    const url = (req.reverse_proxy as string | undefined) || "https://api.together.xyz/v1/completions"
