...
export class MistralAIAdapter implements ChatCompletionAdapter {
  source = "mistralai" as const

  async forwardRequest(req: ChatCompletionRequest): Promise<Response> {
    const url = (req.reverse_proxy as string | undefined) || "https://api.mistral.ai/v1/chat/completions"
