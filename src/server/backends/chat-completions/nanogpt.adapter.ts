...
export class NanogptAdapter implements ChatCompletionAdapter {
  source = "nanogpt" as const

  async forwardRequest(req: ChatCompletionRequest): Promise<Response> {
    const url = (req.reverse_proxy as string | undefined) || "https://api.nanogpt.net/v1/chat/completions"
