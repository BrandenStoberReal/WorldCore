...
export class SiliconflowAdapter implements ChatCompletionAdapter {
  source = "siliconflow" as const

  async forwardRequest(req: ChatCompletionRequest): Promise<Response> {
    const url = (req.reverse_proxy as string | undefined) || "https://api.siliconflow.cn/v1/chat/completions"
