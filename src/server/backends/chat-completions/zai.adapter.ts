...
export class ZaiAdapter implements ChatCompletionAdapter {
  source = "zai" as const

  async forwardRequest(req: ChatCompletionRequest): Promise<Response> {
    const url = (req.reverse_proxy as string | undefined) || "https://open.bigmodel.cn/api/paas/v4/chat/completions"
