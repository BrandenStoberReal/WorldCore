...
describe("remoteCount", () => {
  it("falls back to estimate on connection error", async () => {
    const mockFetch = async (): Promise<Response> => ({ json: async () => ({}), ok: false, status: 500, headers: new Headers(), redirected: false, statusText: "", body: null, bodyUsed: false, type: "basic" as ResponseType, url: "" } as Response)
