...
function parseChunks(
  pngData: Buffer,
): { offset: number; length: number; type: string; data: Buffer; crc: number }[] {
...
export async function readCharacterCardJson(
  filePath: string,
...
export function removePngTextChunk(buffer: Buffer, keyword: string): Buffer {
  ...
  const filtered = chunks.filter(
    (c) =>
      !(
        c.type === 'tEXt' &&
        c.data.indexOf(Buffer.from(keyword, 'latin1')) === 0 &&
        (c.data[keyword.length] ?? 0) === 0
      ),
  );
