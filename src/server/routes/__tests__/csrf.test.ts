...
describe("BUG-17: CSRF token consistency", () => {
  ...
  it("login response token matches session cookie token", async () => {
    ...
    await db.insert(users).values({
      id: TEST_USER, handle: TEST_USER, name: "Test",
      passwordHash: hash, role: "user", enabled: true, createdAt: Date.now(),
    })
    ...
    try {
      ...
      const cookieMatch = setCookie.match(new RegExp(`${SHARED_CONST.SESSION_COOKIE_NAME}=([^;]+)`))
