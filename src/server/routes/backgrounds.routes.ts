...
export const backgroundsRoutes = {
  upload: errorGuard(async (req: Request): Promise<Response> => {
    const formData = await req.formData()
    const result = await assetService.upload(formData, "backgrounds")
  ...
  list: errorGuard(async (_req: Request): Promise<Response> => {
    const result = await assetService.list("backgrounds")
    return Response.json(result)
  }),

  delete: errorGuard(async (req: Request): Promise<Response> => {
    const body = (await req.json()) as { fileName: string }
    await assetService.delete("backgrounds", body.fileName)
