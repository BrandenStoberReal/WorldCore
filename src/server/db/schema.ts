...
export const groups = sqliteTable(
  'groups',
  {
    ...
    allowSelfResponses: integer('allow_self_responses', { mode: 'boolean' })
      .notNull()
      .default(false),
...
export const worldinfoFiles = sqliteTable(
  'worldinfo_files',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
  ...
  (table) => [index('worldinfo_files_user_id_idx').on(table.userId)],
);
...
export const worldinfoEntryStates = sqliteTable(
  'worldinfo_entry_states',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
    chatId: text('chat_id').notNull(),
    entryUid: text('entry_uid').notNull(),
    entryFileId: integer('entry_file_id').references(() => worldinfoFiles.id, {
      onDelete: 'cascade',
    }),
...
export const secrets = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex('secrets_key_user_id_idx').on(table.key, table.userId),
  ],
);
...
export const stats = sqliteTable(
  ...
  (table) => [index('stats_user_id_idx').on(table.userId)],
);
...
export const imageMetadata = sqliteTable(
  'image_metadata',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
    fileName: text('file_name').notNull().unique(),
  ...
  (table) => [index('image_metadata_user_id_idx').on(table.userId)],
);
...
export const quickReplies = sqliteTable(
  ...
  (table) => [index('quick_replies_user_id_idx').on(table.userId)],
);
...
export const vectorStores = sqliteTable(
  'vector_stores',
  {
    id: text('id').primaryKey(),
    name: text('name').notNull(),
  ...
  (table) => [index('vector_stores_user_id_idx').on(table.userId)],
);
