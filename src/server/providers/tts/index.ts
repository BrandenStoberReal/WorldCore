...
export async function synthesizeTTS(
  ...
  switch (provider) {
    ...
    case "bark": {
      ...
      if (!secret) throw new ApiError("MISSING_SECRET", "huggingface_key not configured for bark", 500);
    ...
    case "silero": {
      const secret = await secretManager.read("huggingface_key" as SecretKey);
      if (!secret) throw new ApiError("MISSING_SECRET", "huggingface_key not configured for silero", 500);
...
<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="en-US">
  ...
  const ttsUrl = (configData["TTSUrl"] as string | undefined) ?? "https://speech.platform.bing.com/consumer/speech/synthesize/readaloud/edge/v1";
...
async function synthesizeSilero(
  ...
  const res = await fetch("https://api-inference.huggingface.co/models/sileroai/silero-wav2vec2-large", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${apiKey}`,
