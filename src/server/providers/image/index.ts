...
async function callFalAi(apiKey: string, options: GenerateOptions): Promise<ImageResult[]> {
  const {
    prompt,
    negativePrompt,
    width = 512,
...
async function callPollinations(options: GenerateOptions): Promise<ImageResult[]> {
...
async function callStability(apiKey: string, options: GenerateOptions): Promise<ImageResult[]> {
  const {
    prompt,
    negativePrompt,
    width = 512,
...
async function callHuggingface(apiKey: string, options: GenerateOptions): Promise<ImageResult[]> {
  const {
    prompt,
    negativePrompt,
    width = 512,
  ...
  switch (provider) {
    ...
    case 'comfyrunpod': {
      ...
      return callOpenaiCompatibleImage(
        'comfyrunpod',
        secret.value,
        'https://rs6743vf6cfcujdk.us-east-1.aws.replicate.runpod.net',
        options,
      );
    }
    case 'together': {
