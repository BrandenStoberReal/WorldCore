...
export async function generateComfy(
  ...
  const workflow = JSON.parse(JSON.stringify(DEFAULT_COMFY_WORKFLOW)) as Record<string, Record<string, unknown>>;
  ...
  const promptHistory = (historyData[promptId] as Record<string, unknown> | undefined)?.["outputs"] as Record<string, unknown> | undefined;
  if (promptHistory) {
