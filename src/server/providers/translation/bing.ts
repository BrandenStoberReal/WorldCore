export async function translateBing(
  ...
  const res = await fetch(
    'https://api-edge.cognitive.microsofttranslator.com/translate?from=&to=' + targetLang,
    {
      method: 'POST',
      headers: {
  ...
  return (
    ((data[0]?.translations as Array<Record<string, unknown>> | undefined)?.[0]?.text as string) ||
    text
  );
}
