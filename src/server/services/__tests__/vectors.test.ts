...
import type { VectorEmbeddingRequest, VectorSearchRequest, VectorUpsertRequest, VectorDeleteRequest } from "@/shared/types/vectors"
...
describe("VectorService", () => {
  ...
  describe("upsert", () => {
    ...
    it("upserts vectors with metadata", async () => {
      ...
      const req: VectorUpsertRequest = {
        vectors: [
          { id: "v1", values: embedResult.embeddings[0]!, metadata: { source: "test" } },
        ],
      }
      const result = await service.upsert(req)
  ...
  describe("search", () => {
    it("returns results sorted by similarity", async () => {
      const embedResult = await service.embed({ texts: ["alpha", "beta", "gamma"], model: "default" })
      const req: VectorUpsertRequest = {
        vectors: [
          { id: "v1", values: embedResult.embeddings[0]! },
          { id: "v2", values: embedResult.embeddings[1]! },
    ...
    it("returns metadata with search results", async () => {
      ...
      const req: VectorUpsertRequest = {
        vectors: [
          { id: "v1", values: embedResult.embeddings[0]!, metadata: { tag: "important" } },
        ],
      }
      await service.upsert(req)
      const searchReq: VectorSearchRequest = {
