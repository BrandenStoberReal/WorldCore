...
describe('PresetService', () => {
  it('save and get back', async () => {
    ...
    expect((result!.data as Record<string, unknown>).content).toBe('You are a helpful assistant.');
  });
  ...
  it('getAll with category filter', async () => {
    ...
    const found = all.find((p) => (p.data as Record<string, unknown>).name === 'test-preset-t14');
    expect(found).not.toBeNull();
  });

  it('export returns buffer and filename', async () => {
    const result = await presetService.exportPreset('sysprompt', 'test-preset-t14');
    expect(result).not.toBeNull();
