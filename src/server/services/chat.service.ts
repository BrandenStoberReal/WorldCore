...
export class ChatService {
  ...
  async editMessage(fileId: string, messageIndex: number, updates: Partial<ChatMessage>): Promise<void> {
    ...
    if (messageIndex < messages.length) {
      ...
      await writeJsonl<ChatMetadata | ChatMessage>(filePath, [allLines[0] as ChatMetadata, ...messages])
      await this.updateChatStats(fileId)
    }
  }
  ...
  async rename(fileId: string, newName: string): Promise<void> {
    ...
    await writeJsonl<ChatMetadata | ChatMessage>(filePath, [metadata, ...allLines.slice(1) as ChatMessage[]])
  }
  ...
  async listGroupChats(groupId: string): Promise<ChatInfo[]> {
    ...
    for (const file of files) {
      ...
      if (metadata && typeof (metadata as Record<string, unknown>).group === "string" && (metadata as Record<string, unknown>).group === groupId) {
